...
# First, install remotes if don't have it
...
brain_liver_meta <- zebrafish_meta_data_df %>%
  ...
  mutate(
    Condition = case_when(
      Condition == "brain" ~ "case",     # ImpulseDE2 needs a "Condition" column with "case" and "control" values
      Condition == "liver" ~ "control",  # Necessary for pairwise comparison
      TRUE ~ Condition # Keep other conditions as they are (for now)
    ),
    # ImpulseDE2 needs a "Time" column filled with numeric values
...
zebrafish_ImpulseDE2 <- runImpulseDE2(
  ...
  boolCaseCtrl    = TRUE,                        # Case control analysis. To analyze either one sample - FALSE, or pairwise comparison - TRUE.
...
# Get ranked gene list from ImpulseDE2 results and organize based on adjusted p-value
results_df <- zebrafish_ImpulseDE2$dfImpulseDE2Results

# Replace any adjusted p-values of 0 with a very small positive number to avoid -log10(0) = Inf
...
results_df$padj[results_df$padj <= 0] <- min_padj_value_non_zero 

# Convert Ensemble ID to Entrez IDs (GSEA prefers EntrezID)
...
# Perform GSEA for Gene Ontology (Biological Process)
